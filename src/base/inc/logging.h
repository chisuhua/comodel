...
class Logger
{
  public:
    ...
    {
        auto ss = GetFormatString(format, args...);
        print(loc, ss);
    }
...
#define info(...) base_message(::Logger::getInfo(), __VA_ARGS__)
#define hack(...) base_message(::Logger::getHack(), __VA_ARGS__)

#define warn_once(...) base_message_once(::Logger::getWarn(), __VA_ARGS__)
#define info_once(...) base_message_once(::Logger::getInfo(), __VA_ARGS__)
