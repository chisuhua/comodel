...
// D.u = S0.u.
void Instruction::S_MOV_B64(ThreadItem *item)
{
...
// D.u = S0.u.
void Instruction::S_MOV_B32(ThreadItem *item)
{
	Register s0;
...
void Instruction::S_NOT_B32(ThreadItem *item)
{
	Register s0;
	Register nonzero;
...
void Instruction::S_WQM_B64(ThreadItem *item)
{
	ISAUnimplemented(item);
}

// D.u = PC + 4, PC = S0.u
void Instruction::S_SWAPPC_B64(ThreadItem *item)
{
	Register s0_lo;
	Register s0_hi;
 ...
 * non-zero. */
void Instruction::S_AND_SAVEEXEC_B64(ThreadItem *item)
{
	// Assert no literal constant with a 64 bit instruction.
	assert(!(opcode.ssrc0 == 0xFF));
