...
void Module::Module(const ModuleParams *p) 
    : ClockedObject(p)
{
    m_response_queue = make_shared<MsgQueue<MsgPtr>>();
    // m_msg_active_list = make_shared<MsgQueuePtr>();
}
...
void Module::FetchInput() {
    ...
    // LOG(INFO) << "Module " << m_name << " FetchInput" ;
    MsgPtr msg = PopQueue();
    if (msg) {
...
bool Module::Execute() {
    ...
    // LOG(INFO) << "Module " << m_name << " Execute" ;
    if (m_msg_active_list.empty()) {
